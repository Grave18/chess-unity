...
namespace Chess3D.Runtime.LobbyManagement {
    [System.Serializable]
    public struct Lobby {
        public string Name;
        public bool IsValid;
        ...
        public bool HasChanged(Lobby @new) {
            ...
            for(int i = 0; i < @new.Members.Count; i++) {
                ...
                if(newMember.Id != oldMember.Id || newMember.IsReady != oldMember.IsReady || newMember.Name != oldMember.Name || newMember.Image != oldMember.Image)
                    return true;
            }
