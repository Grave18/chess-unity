...
using Chess3D.Runtime.Menu.LobbyManagement.Providers;
using LobbyManagement;
using UnityEditor;
using UnityEngine;

namespace Chess3D.Runtime.Menu.LobbyManagement.Editor
{
                ...
                foreach (var member in currentRoom.Members)
                {
                    ...
                    if (memberFoldouts[member.Id])
                    {
                        ...
                        if (member.Image != null)
                        {
                            ...
                            rect.x += indentOffset;
                            rect.width = 64;
                            EditorGUI.DrawPreviewTexture(rect, member.Image);
                        }
                        EditorGUI.indentLevel--;
                    }
                }
